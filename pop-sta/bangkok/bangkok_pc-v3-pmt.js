...
// accident and police station insert

fetch('./fstyle4popsta.json')
  .then(res => res.json())
  .then(style => {
    // JS側でスタイルにレイヤーを追加
    style.sources['accident'] = {
        "type": "geojson",
        "data": "./bangkok/accident.geojson",
        };
    style.sources['police-station'] = {
        "type": "geojson",
        "data": "./bangkok/police.geojson",
        };
    style.sources['riskptimage'] ={
        "type": "image",
        "url": "./bangkok/riskpoint_modified.png",
        "coordinates": [ 
             [100.4520365953662520,13.8798600309613729],
             [100.7628251238276960,13.8798600309613729],
             [100.7628251238276960,13.7088129138373365],
             [100.4520365953662520,13.7088129138373365],
        ]      
    }
    style.layers.push({
        "id": "riskptimage_Layer",
        "type": "raster",
        "source": "riskptimage",
        "paint": {
            "raster-opacity": 0.5,
...
// accident and police station insert end
...
// accident and police station layer
const layonoffa1 = document.getElementById("accident-Layer");
const layonoffa2 = document.getElementById("police-Layer");
const layonoffa3 = document.getElementById("riskptimage-Layer");
...
// accident and police station layer control
layonoffa1.addEventListener('click', () => {
    if (layonoffa1.checked == true) {
        map.setLayoutProperty("accident_Layer", "visibility", "visible");
    } else {
        map.setLayoutProperty("accident_Layer", "visibility", "none");
    }
});
layonoffa2.addEventListener('click', () => {
    if (layonoffa2.checked == true) {
        map.setLayoutProperty("police_Layer", "visibility", "visible");
    } else {
        map.setLayoutProperty("police_Layer", "visibility", "none");
    }
});
layonoffa3.addEventListener('click', () => {
    if (layonoffa3.checked == true) {
        map.setLayoutProperty("riskptimage_Layer", "visibility", "visible");
    } else {
        map.setLayoutProperty("riskptimage_Layer", "visibility", "none");
    }
});
